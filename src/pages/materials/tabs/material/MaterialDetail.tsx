...
export default function MaterialDetail() {
  ...
  const thumbnails = useMemo(
    () => Array.from({ length: THUMBNAIL_COUNT }, (_, i) => images[i] ?? null),
    [images],
  );

  if (loading) return <Loading overlay fullScreen />;
  ...
  return (
    ...
    >
      ...
      >
        <Row
          gutter={[32, 24]}
          style={{ height: "100%", flexWrap: isMobile ? "wrap" : "nowrap" }}
        >
          <Col xs={24} md={7} style={{ overflow: "hidden" }}>
            <div className="flex flex-col h-full gap-4">
              <ZoomableImage
                src={
                  images[selectedIndex]
